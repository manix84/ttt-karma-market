...
- Supports Classic TTT and TTT2, with a conservative popup display path for TTT2.
...
- `ttt_karma_market_ttt2_force_popup`: prefers the popup in TTT2 instead of injecting into TTT2's round-end UI.
...
Primary target: TTT2 and Classic TTT.

The code is namespaced under `TTTKarmaMarket` and split into shared, server, and
client modules. It uses standard TTT round hooks and karma accessors, while
TTT2 defaults to popup display to avoid depending on the redesigned TTT2
round-end screen internals.
